...
//This file has been modified by retep998 and is not the original software
...
namespace { // anonymous namespace for local linkage
    ...
    uint8_t gf2_8_inv[256];
    uint8_t byte_sub[256];
    uint8_t inv_byte_sub[256];
...
#define GetByte(a,n) ((uint8_t)((a) >> (n<<3)))
...
#define RotByteL(a) _rotl(a,8)
    ...
    inline uint8_t GF2_8_mult(uint8_t a, uint8_t b)
    ...
    {
        uint32_t i;
        uint8_t a1,a2,a3,b1,b2,b3,b4,b5;
        for (i = 0; i < 256; i++)
        {
            a1 = byte_sub[i];
            a2 = xmult(a1);
...
#define InvFinalRound8(d,s) compute_one_final_inv(d,s,0,1,3,4,8); \
    ...
    void DumpCharTable(ostream & out, const char * name, const uint8_t * table, int length)
    ...
    void DumpHex(const uint8_t * table, int length)
...
void AES::StartEncryption(const uint8_t * key)
{
    KeyExpansion(key);
} // StartEncryption

void AES::EncryptBlock(const uint8_t * datain1, uint8_t * dataout1)
